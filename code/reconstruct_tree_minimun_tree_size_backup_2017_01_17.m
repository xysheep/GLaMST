...
%     % for each components, find the root of it. This component is grown from which reconstructed node? 
%     parent_ind = zeros(1,size(components,2));
%     for i=1:size(components,2)
%         parent_ind(i) = find(sum(mst_adj(:,components(:,i)==1),2)~=0 & reconstructed_indicator==1);
%     end
%     merged_parent_ind = unique(parent_ind);
%     merged_components = zeros(size(components,1), length(merged_parent_ind));
%     for i=1:length(merged_parent_ind)
%         merged_components(:,i) = sum(components(:,parent_ind==merged_parent_ind(i)),2);
%     end
...
% remove unnecessary nodes and edges
fprintf('    Rewire the tree to further reduce size ... \n'); 
while 1
    ...
    if pairwise_dist(rewire_destination, node_to_rewire)==1
        ...
        % [~,~,~,edit_operations, edit_dist, ~, ~] = EditDistance_all_fastest(all_sequences{rewire_destination},all_sequences{node_to_rewire});
        [~,I] = min(edit_dist);
        edit_operations = edit_operations{I};
        best_parent = rewire_destination;
        for i=1:length(edit_operations) 
            [~,~,~,edit_operations, edit_dist, ~, ~] = EditDistance_all_faster(all_sequences{best_parent},all_sequences{node_to_rewire});
            % [~,~,~,edit_operations, edit_dist, ~, ~] = EditDistance_all_fastest(all_sequences{best_parent},all_sequences{node_to_rewire});
